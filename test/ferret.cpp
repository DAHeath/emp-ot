...
void test_ferret(int party, NetIO *ios[threads+1]) {
  ...
  FerretCOT<threads> * ferretcot = new FerretCOT<threads>(party, ios);
  ...
  int num = 1 << 24;
  cout <<"Active FERRET RCOT\t"<<double(num)/test_rcot<FerretCOT<threads>>(ferretcot, ios[0], party, num, false)*1e6<<" OTps"<<endl;
  ...
  cout <<"Active FERRET RCOT inplace\t"<<double(batch_size)/test_rcot<FerretCOT<threads>>(ferretcot, ios[0], party, batch_size, true)*1e6<<" OTps"<<endl;
  delete ferretcot;
}
