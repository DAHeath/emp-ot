...
struct FerretCOT {
  ...
  static FerretCOT make(NetIO& io, GT::PRG& prg) {
    FerretCOT out;
    ...
    out.lpn_extension(PRE, io, prg, out.small_correlation, init);

    return out;
  }

  std::size_t extend(NetIO& io, GT::PRG& prg, std::span<std::bitset<128>> buf) {
    ...
    for (std::size_t i = 0; i < round; ++i) {
      lpn_extension(REGULAR, io, prg, buf, small_correlation);
  ...
  std::vector<std::bitset<128>> extend(NetIO& io, GT::PRG& prg, std::size_t n) {
