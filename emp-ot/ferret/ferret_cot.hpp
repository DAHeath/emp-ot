...
template<typename T, int threads>
void FerretCOT<T, threads>::extend(
    int party, int n, int k, ThreadPool* pool, NetIO* io,
  ...
  if (party == ALICE) {
    lpn<Role::Sender>(n, k, pool, io, pool->size(), ot_output, ot_input+mpcot->consist_check_cot_num);
  } else {
    lpn<Role::Receiver>(n, k, pool, io, pool->size(), ot_output, ot_input+mpcot->consist_check_cot_num);
  }
}
...
void FerretCOT<T, threads>::extend_f2k(block *ot_buffer) {
  ...
  extend(party, n, k, pool, io, ot_buffer, mpcot, pre_ot, ot_pre_data);
...
void FerretCOT<T, threads>::setup(std::string pre_file) {
  ...
  if(file_exists(pre_ot_filename) == true) {
    ...
    extend(party, n_pre, k_pre, pool, io, ot_pre_data, &mpcot_ini, &pre_ot_ini, pre_data_ini);
  }
